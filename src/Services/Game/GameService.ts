...
export default class GameService {
    ...
    async completeGame(user: User): Promise<void>
    {
        ...
        if (playersCompletedGame === 3) {
            ...
            await this.notificationService.broadcastToPlayers(activePlayers.map(u => u.discord_user_id), "Три первых игрока прошли игру, а потому призовые места закончились. Но вы все еще можете посоревноваться в прохождении игры на скорость! Также, если у вас слишком долго не получается решить квест, вы можете обратиться к боту за подсказкой командой `!подсказонька`. Помните, что взятие подсказки (или нескольких) на каждом этапе накинет вам штрафное время. Посмотреть список лидеров вы можете командой `!лидеры`. Удачи!");
